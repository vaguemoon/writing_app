...
def create_package(characters_to_pack, output_filepath, status_queue):
    ...
    try:
        ...
        try:
            ...
            status_queue.put(f"警告：無法下載函式庫 ({e})。將用 CDN 連結。")
            hanzi_writer_lib = None

        # 2. 清理硬編碼的 audio 路徑
        status_queue.put("狀態：清理硬編碼路徑...")
        modified_html = re.sub(r'<audio id="completion-sound".*?</audio>', '', html_template)

        # 3. 注入預載資料 (確保在 app.js 執行前定義)
        status_queue.put("狀態：注入預載資料...")
        escaped_chars = json.dumps(characters_to_pack, ensure_ascii=False)
        data_script = f"""<script>
      window.preloadedCharacters = {escaped_chars};
    </script>"""
        
        # 我們將資料注入點放在 <script src="app.js"> 之前
        ...
        # 4. 內嵌 CSS 和 JS
        status_queue.put("狀態：內嵌 CSS 與 JavaScript...")
        modified_html = modified_html.replace(
        ...
        if original_header_match:
            ...
            if theme_switch_match:
                # 如果找到主題切換器，就把它加到新標題後面
                new_header_content += '\n' + theme_switch_match.group(1)
            
            # 替換整個 header 的內部 HTML
